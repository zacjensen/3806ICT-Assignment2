...
def plot_train(agent, rtrace, steps, trace, start, env):
    ...
    plot_trace(agent, start, trace, env, title="trace of the last episode")
...
def main(params, plot=False):
    if (len(params) < 3):
        print("Usage: 'python RLSolver.py width height' \nExample: 'RLSolver.py 10 10' ")
        sys.exit()
    width = int(params[1])
    height = int(params[2])
    ...
    if plot:
        plot_train(agent, rtrace, steps, trace,
                   start_position, env)  # Run the training

    # Now for getting the optimal trace
